#!/usr/bin/env python
"""Build and deploy the CodeForge Lambda package.
...
def build(staging: pathlib.Path) -> bytes:
    ...
    buf = pathlib.Path(tempfile.gettempdir()) / "codeforge-swe-team-lambda.zip"
...
def main() -> int:
    ...
    if args.dry_run:
        out = pathlib.Path(tempfile.gettempdir()) / "codeforge-swe-team-lambda.zip"
        print("dry-run: %s" % out)
        return 0
