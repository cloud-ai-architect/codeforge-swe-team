...
# The sandbox stage starts Fargate tasks. It needs PassRole for the two
# sandbox roles specifically -- a broad PassRole would let this function hand
# any role to a task it starts, which is the classic privilege-escalation
# path out of a deploy-adjacent Lambda.
data "aws_iam_policy_document" "lambda_sandbox" {
  statement {
    sid    = "SandboxExecution"
    effect = "Allow"
    ...
    condition {
      ...
      values   = ["arn:aws:ecs:*:*:cluster/${var.name_prefix}-sandbox"]
    }
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:iam::*:role/${var.name_prefix}-sandbox-execution",
      "arn:aws:iam::*:role/${var.name_prefix}-sandbox-task",
    ]
  ...
  statement {
    ...
    resources = ["arn:aws:logs:*:*:log-group:/ecs/${var.name_prefix}-sandbox:*"]
  }
}

resource "aws_iam_role_policy" "lambda_sandbox" {
